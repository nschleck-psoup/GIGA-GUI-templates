/*
  ...
  Demonstrated example functions include a vertical interface, global styles, linked object states, grids and sub-grids, and event handlers
...
// declare GUI screens
  lv_obj_t * screen1; // sequence screen
  lv_obj_t * screen2; // manual screen
...
// LVGL create object Functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  lv_obj_t * createGrid1(lv_obj_t * screen){ // Create Top-Level Grid
  ...
  lv_obj_t * createGrid2(lv_obj_t * topLevelGrid){ // Create Sub-Grid 1
  ...
  lv_obj_t * createTopbar_scr1(lv_obj_t * grid, char * header){
    lv_obj_t * cell;
    lv_obj_t * label;
    lv_obj_t * dropdown;
    lv_obj_t * btn;
    ...
    // base container
      ...
      lv_obj_set_grid_cell(cell, LV_GRID_ALIGN_STRETCH, 0, 1,    //column
  ...
  lv_obj_t * createTopbar_scr2(lv_obj_t * grid, char * header){
    lv_obj_t * cell;
    lv_obj_t * label;
    lv_obj_t * dropdown;
    lv_obj_t * btn;
  ...
  lv_obj_t * createButtonPanel(lv_obj_t * grid){
    lv_obj_t * subGrid;
    lv_obj_t * label;
    lv_obj_t * btn;
    lv_obj_t * slider;
    ...
    // Base container (sub-grid)
      ...
      lv_obj_set_grid_dsc_array(subGrid, col_dsc, row_dsc);
      lv_obj_set_grid_cell(subGrid, LV_GRID_ALIGN_STRETCH, 0, 1,    //column
                        LV_GRID_ALIGN_STRETCH, 0, 1);     //row
      // styling
      lv_obj_set_style_pad_row(subGrid, 0, 0);
      lv_obj_set_style_pad_column(subGrid, 0, 0);
      lv_obj_set_style_pad_left(subGrid, 30, 0);
      lv_obj_set_style_pad_right(subGrid, 30, 0);
      lv_obj_clear_flag(subGrid, LV_OBJ_FLAG_SCROLLABLE);

    // create power button
      lv_obj_t * powerBtn = lv_btn_create(subGrid);
    ...
    // create dropdown menu
      dropdown = lv_dropdown_create(subGrid);
    ...
    // create charge button
      // btn = lv_btn_create(subGrid);
      // lv_obj_set_grid_cell(btn, LV_GRID_ALIGN_CENTER, 2, 1, LV_GRID_ALIGN_CENTER, 1, 1);
      ...
      // lv_obj_center(label);   
      // label = lv_label_create(subGrid);
      // lv_obj_set_grid_cell(label, LV_GRID_ALIGN_CENTER, 2, 1, LV_GRID_ALIGN_CENTER, 2, 1);
      // lv_label_set_text(label, "Send\nCharge");
      // lv_obj_add_style(label, &style_subtitle_label, 0);

      // lv_obj_clear_flag(btn, LV_OBJ_FLAG_CLICKABLE);
      // lv_obj_add_event_cb(powerBtn, powerToggle_event, LV_EVENT_CLICKED, btn);  // link charge button to power button status

    return subGrid;
  }

  lv_obj_t * createSwitchPanel(lv_obj_t * grid){
    lv_obj_t * subGrid;
    lv_obj_t * label;
    lv_obj_t * swtch;
    lv_obj_t * switchPanel;
    ...
    // Base container (sub-grid)
      ...
      lv_obj_set_grid_dsc_array(subGrid, col_dsc, row_dsc);
      lv_obj_set_grid_cell(subGrid, LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_STRETCH, 1, 1);
    ...
    // switches/sliders
      // lv_obj_t * switch_wifi =      createSwitch(subGrid, 1, "Wifi", LV_SYMBOL_WIFI);
      lv_obj_t * slider_speed =     createSlider(subGrid, 1, "Speed",     LV_SYMBOL_CHARGE);
      lv_obj_t * slider_duration =  createSlider(subGrid, 2, "Duration",  LV_SYMBOL_REFRESH);
      lv_obj_t * slider_overlap =   createSlider(subGrid, 3, "Overlap",   LV_SYMBOL_LIST);
      lv_obj_t * switch_bluetooth = createSwitch(subGrid, 4, "Bluetooth", LV_SYMBOL_BLUETOOTH);
      lv_obj_t * switch_eye =       createSwitch(subGrid, 5, "Visible",   LV_SYMBOL_EYE_CLOSE);
      // lv_obj_t * switch_storage =   createSwitch(subGrid, 4, "Storage", LV_SYMBOL_SD_CARD);
      // lv_obj_t * switch_shuffle =   createSwitch(subGrid, 5, "Shuffle", LV_SYMBOL_SHUFFLE);

      lv_obj_add_event_cb(switch_eye, switchToggle_eye_event, LV_EVENT_CLICKED, 0);
      

    return subGrid;
  }

  lv_obj_t * createSwitch(lv_obj_t * parent, int row, char * title, char * symbol){
    lv_obj_t * panel;
    lv_obj_t * swtch;
    lv_obj_t * label;
    lv_obj_t * icon;
  ...
  lv_obj_t * createSlider(lv_obj_t * parent, int row, char * title, char * symbol){
    lv_obj_t * panel;
    lv_obj_t * slider;
    lv_obj_t * label;
    lv_obj_t * icon;
...
// LVGL/misc helper functions ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  static void switchToggle_eye_event(lv_event_t * e){ // specific switch event handler: eye open/closed symbol based on switch state
    lv_obj_t * swtch = lv_event_get_target(e);
    lv_obj_t * panel = lv_obj_get_parent(swtch);
  ...
  void pageBtn_event_cb(lv_event_t * e){
